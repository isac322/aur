...
## 5. 유지보수 정기 작업: "버전업" 및 "의존성 최신화"

사용자가 "버전업", "버전업해줘", "전체 최신화", "의존성 최신화", "버전 업데이트", "upstream 확인" 등을 요청하면 아래 프로토콜을 따른다. 과거 캐시나 이전 실행 목록을 신뢰하지 않고, 항상 워크스페이스의 모든 패키지(전수)와 공식 upstream 원격을 실시간 전수조사한다.
